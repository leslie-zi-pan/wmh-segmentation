...
from src.enums import (
    AMSTERDAM_RAW_VOLUME_TESTING_IMG_PATHS,
...
def get_train_raw_paths():
    raw_train_paths = image_directories_handler(
        SINGAPORE_RAW_VOLUME_TRAINING_IMG_PATHS, CountryDirType.Singapore
    )
    raw_train_paths += image_directories_handler(
        UTRECHT_RAW_VOLUME_TRAINING_IMG_PATHS, CountryDirType.Utrecht
    )
    raw_train_paths += image_directories_handler(
        AMSTERDAM_RAW_VOLUME_TRAINING_IMG_PATHS, CountryDirType.Amsterdam
    )
...
def get_test_raw_paths():
    raw_test_paths = image_directories_handler(
        SINGAPORE_RAW_VOLUME_TESTING_IMG_PATHS, CountryDirType.Singapore
    )
    raw_test_paths += image_directories_handler(
        UTRECHT_RAW_VOLUME_TESTING_IMG_PATHS, CountryDirType.Utrecht
    )
    raw_test_paths += image_directories_handler(
        AMSTERDAM_RAW_VOLUME_TESTING_IMG_PATHS, CountryDirType.Amsterdam
    )
...
def slice_volume_handler(raw_data_paths, interim_dir_dict):
    for index, filedict in enumerate(raw_data_paths):
        ...
        if (
            num_slices_image_t1
            != num_slices_label & num_slices_image_flair
            != num_slices_label
        ):
            raise ValueError(
                f"Image and Label dim not match: {num_slices_image_t1.shape}, {label.shape}"
            )
